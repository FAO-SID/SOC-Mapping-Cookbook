\clearpage
## Model Selection in Digital Soil Mapping
*M Guevara & GF Olmedo*
...
```{r }
#Load the openair library and the dataset 
library(openair)
...
A perfect model would have a FAC2, r, COE and IOA ~ 1.0, while all the others ~ 0. However, digital soil mappers should have in mind that there is not such thing as a perfect model on digital SOC mapping for large areas, specially if we deal with sparse data from legacy soil profile or pit observations usually collected over long periods of time. Depending on the situation, some performance measures might be more appropriate than others. Hence, there is not a single best measure, and it is necessary to use a combination of the performance measures [@chang2004air]. 

```{r}
modsts <- modStats(modData,obs = "obs", mod = "mod", type = "model")
cbind(modsts[1], round(modsts[-1], 2))
```
