...
Soils provide ecosystem sercives critical to live on Earth. The Food and
...
knowledge and practical experiences gained during the implementaion
...
development it can be applicsable to map various soil properties.
...
depth classes and spatial resolution can be modelled depending on
project and mapping objectives and available input data. For GSOCmap, a
1 km grid is pursued. The same methodology and input data can also be
used to produce higher resolution soil grids.
...
be used to predict the latter with multivariate prediction techniques.
\end{quote}

\hypertarget{preparation}{%
\chapter{Preparation of Local Soil Property Data}\label{preparation}}
...
analysis, soils are then sampled by genetic horizon or by depth class.
...
assessments, soil profile data and analytical information needs to be
stored in a database. This can be a set of simple \emph{Excel}
spreadsheets, or a relational or object-oriented data base management
...
are a useful tool. Tables @ref(tab:example table for site-level) and
@ref(tab:example table for horizon-level) are examples of how soil
information can be stored. The advantage of such organization is the
possibility to develop relational databases which can be easily queried.
Such a systematic approach will support the organization of national
soil information and will reduce errors in future modelling exercises
\citep{baritz2008environmental}.

Table @ref(tab:example table for site-level) stores site-level data,
which describe the location of the soil description and/or sampling
site: spatial coordinates, landscape attributes such as slope gradient
and slope form, soil class, land cover type, rock type, etc. In this
table every row should hold a single soil profile. One column, usually
the first one, should be the soil profile's unique identifier. Using the
latter, soil information can be easily linked from one table to another.

\begin{table}

\caption{(\#tab:example table for site-level)Example for site-level data table}
\centering
...
Table @ref(tab:example table for horizon-level) stores information from
the soil description, such as horizon name, horizon thickness, organic
matter content, carbonate content, soil color, laboratory soil analysis,
etc. The first column contains the soil profile's unique identifier. It
is important to include the upper and lower limits for each soil layer;
in case the sampling strategy deviates from soil layers/soil horizons,
the upper and lower depth of the sampling locations should be specified
if possible. This information is needed for modelling soil properties
over the soil profile.

\begin{table}

\caption{(\#tab:example table for horizon-level)Example for profile-description table}
\centering
...
\ref{PreparationProfiles}. The second example is using top soil or auger
...
properties can be evaluated and modelled using this cookbook as well.
...
\item
  ...
  complete recovery. No correction against the mineral carbon is needed.
  Wet oxidation should typically only be applied to samples with
  \(<5\%\) organic matter.
\end{itemize}
...
2.2) or from sampling and analysing horizons and/or fixed depths. Since
...
For both \textbf{Options 2} and \textbf{3}, transformation is needed,
using e.g.~equal-area splines. In the case of \textbf{Option 2}, the use
of equal-area splines was first proposed by \cite{ponce1986improved},
and later tested against real data \citep{bishop1999modelling}. This
technique is based on fitting continuous depth functions for modelling
the variability of soil properties with depth. Thus, it is possible to
convert soil profiles to standard depths, but also to fill gaps. The
equal-area spline function consists of a series of local quadratic
polynomials that join at so-called knots, located at the horizon
boundaries, thereby the mean value of each horizon is maintained by the
spline fit. They are called equal-area splines because the area to the
left of the fitted spline curve is equal to the area to the right of the
curve. In case of \textbf{Option 3}, additional information on the
vertical distribution of carbon in the soils is required for accurate
...
@ref(tab:example table for horizon-level)) can be transformed to a SPC.
...
\hypertarget{run-mass-preserving-splines-for-all-the-needed-propierties}{%
\subsubsection{Run mass preserving splines for all the needed
propierties}\label{run-mass-preserving-splines-for-all-the-needed-propierties}}
...
\KeywordTok{plot}\NormalTok{(}\KeywordTok{density}\NormalTok{(dat}\OperatorTok{$}\NormalTok{OCSKGM), }
     \DataTypeTok{main=}\StringTok{'Statistical distribution of original values'}\NormalTok{)}

\KeywordTok{plot}\NormalTok{(}\KeywordTok{density}\NormalTok{(dat}\OperatorTok{$}\NormalTok{OCSKGMlog), }
     \DataTypeTok{main=}\StringTok{'Statistical distribution of log transformed values'}\NormalTok{)}
...
This cookbook focuses on SOC modelling using open source digital mapping
...
provides a wide variety of statistical (e.g.~linear modelling,
...
Variogram modelling with simple, ordinary and universal point or block
...
package. Alternative to using Proj4 character strings, we can use the
...
\CommentTok{# check the cooridnate system:}
...
standard GIS environment. See the help file for writting a raster
...
\item
  \emph{ArcGIS}: 60 day trial can be downloaded at
  \url{http://www.esri.com/software/arcgis/free-trial} (needs
  registration)
\end{itemize}
...
\caption{(\#tab:details of covariates)Code and name of example covariates provided with the cookbook.}
\centering
\begin{tabular}[t]{l|l}
\hline
...
geochemistry has essential impact on the soil chemistry, e.g.~cation
exchange capacity, base saturation, and nutrient stock. The rock
structure determines the ability to disintegrate, which has impact on
...
\item
  \emph{USGS} has several data portals, e.g.~that allow browsing of the
...
\item
  ...
  1.25 million digital polygons that are classified in three levels (a
...
harmonising all your different raster layers. With the stacked rasters
...
\begin{verbatim}
##        id            SOC                BLD         
##  P0003  :   1   Min.   : 0.09358   Min.   :0.05353  
##  P0004  :   1   1st Qu.: 1.76875   1st Qu.:1.28157  
##  P0008  :   1   Median : 2.64157   Median :1.39198  
##  P0009  :   1   Mean   : 3.55929   Mean   :1.38129  
##  P0010  :   1   3rd Qu.: 4.08981   3rd Qu.:1.47554  
##  P0011  :   1   Max.   :86.51037   Max.   :2.92191  
##  (Other):2910                                       
##      CRFVOL            OCSKGM          meaERROR    
##  Min.   : 0.0000   Min.   :0.0111   Min.   :0.172  
##  1st Qu.: 0.0102   1st Qu.:0.7041   1st Qu.:3.160  
##  Median : 5.0000   Median :1.0000   Median :3.840  
##  Mean   :10.9668   Mean   :1.1457   Mean   :3.720  
##  3rd Qu.:17.2730   3rd Qu.:1.4010   3rd Qu.:4.260  
##  Max.   :89.0263   Max.   :7.4302   Max.   :8.770  
##                                                    
##     B04CHE3         B07CHE3         B13CHE3      
##  Min.   :547.2   Min.   :37.60   Min.   : 45.71  
##  1st Qu.:759.6   1st Qu.:43.74   1st Qu.: 60.28  
##  Median :763.9   Median :44.06   Median : 71.27  
##  Mean   :759.8   Mean   :43.89   Mean   : 74.32  
##  3rd Qu.:774.6   3rd Qu.:44.54   3rd Qu.: 79.10  
##  Max.   :801.6   Max.   :45.39   Max.   :167.31  
##  NA's   :2       NA's   :2       NA's   :2       
##     B14CHE3         DEMENV5        LCEE10        PRSCHE3      
##  Min.   :22.26   Min.   :  45.0   1   :1963   Min.   : 424.5  
##  1st Qu.:27.67   1st Qu.: 397.0   2   : 704   1st Qu.: 530.1  
##  Median :30.16   Median : 594.0   3   :  93   Median : 566.0  
##  Mean   :32.29   Mean   : 654.7   4   : 151   Mean   : 604.8  
##  3rd Qu.:35.52   3rd Qu.: 813.8   NA's:   5   3rd Qu.: 653.1  
##  Max.   :60.37   Max.   :2375.0               Max.   :1198.3  
##  NA's   :2       NA's   :2                    NA's   :2       
##     SLPMRG5          TMDMOD3         TMNMOD3     
##  Min.   : 0.000   Min.   :281.0   Min.   :270.0  
##  1st Qu.: 0.000   1st Qu.:291.0   1st Qu.:279.0  
##  Median : 4.000   Median :293.0   Median :280.0  
##  Mean   : 8.458   Mean   :292.3   Mean   :279.7  
##  3rd Qu.:13.000   3rd Qu.:294.0   3rd Qu.:281.0  
##  Max.   :56.000   Max.   :297.0   Max.   :284.0  
##  NA's   :2        NA's   :2       NA's   :2      
##     TWIMRG5          VBFMRG5       VDPMRG5         soilmap   
##  Min.   : 56.00   Min.   :  0   Min.   :-6197   9      :842  
##  1st Qu.: 79.00   1st Qu.:  1   1st Qu.: 3624   3      :432  
##  Median : 94.00   Median : 76   Median : 5716   12     :229  
##  Mean   : 91.06   Mean   :174   Mean   : 5424   10     :227  
##  3rd Qu.:102.00   3rd Qu.:389   3rd Qu.: 7051   14     :195  
##  Max.   :136.00   Max.   :498   Max.   :14058   (Other):969  
...
the soil data and the values of the enviromental covariates in position
...
\item
  ...
  eliminate the point data. This is the case for soilmap layer and the
  30 points.
\end{itemize}
...
classified as convetional upscaling. The first one is class-matching. In
this approach we derive average SOC stocks per class: soil type for
...
\tightlist
\item
  Separate the data base for forests, peat and other land uses If only
...
\item
  If stone content is missing, investigate using other data sources or
  literature, to which a correction for stones should be applied
\item
...
main land cover type (forest, grassland, cropland) 2. Calculate average
SOC stocks from from all soils which match the soil/landscape unit 3.
Present the Soil/landscape map with SOC stocks, do not classify SOC
stocks into groups (e.g. \textless{} 50, 50-100, \textgreater{} 100).
...
\item
  Start `QGIS desktop with GRASS' Figure 8.1 shows the start screen of
  QGIS desktop. In the upper left panel there is the browser panel,
  which lists the geodata used for this example. In the bottom left, the
  layer information is given for the layers displayed on the right.
\end{enumerate}

\begin{figure}
\centering
\includegraphics{images/Conv_upscaling1.png}
\caption{QGIS Desktop with the browser panel on the upper left, the
layer information on the bottom left and the display of your layers on
the right.}
\end{figure}
...
\item
  ...
  `Categorized' (Fig. 8.2). Select the column `WRB' and press the icon
  `Classify' and change the colours if you want. Next, apply the change
  and finish with clicking the OK-button.
\end{enumerate}

\begin{figure}
\centering
\includegraphics{images/Conv_upscaling2.png}
\caption{Load the FYROM Soil Map}
\end{figure}
...
\item
  ...
  3909 which corresponds to MGI 1901/ Balkans zone 7 (Figure 8.3).
\end{enumerate}

\begin{figure}
\centering
\includegraphics{images/Conv_upscaling3.png}
\caption{Project properties and projection settings}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{images/Conv_upscaling4.png}
\caption{Join attributes by location}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{images/Conv_upscaling5.png}
\caption{Example field calculator}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{images/Conv_upscaling6.png}
\caption{Calculate group statistics}
\end{figure}
...
\tightlist
\item
  Join the mean and standard deviation of SOC to the Soil Map. First add
  the files generated during step 6 to the Layers Panels. In the Layers
  Panel, right-click on the FYROM Soil Map. Go to Properties
...
\begin{figure}
\centering
\includegraphics{images/Conv_upscaling7.png}
\caption{Change the legend style to display the SOC values}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{images/Conv_upscaling8.png}
\caption{Example of the Map composer}
\end{figure}
...
\item
  ...
  file with 1 kilometre resolution and projected in lat long degrees
...
\item
  ...
  checkbox for the resampling method and choose Near. This is nearest
  neighbour and most suitable for a transformation of categorical data,
  such as land use (Fig. 8.9).
\end{enumerate}

\begin{figure}
\centering
\includegraphics{images/Conv_upscaling9.png}
\caption{Change the projection of a raster file}
\end{figure}
...
\item
  Next, this layer can be used to continue with the classmatching or
...
\section{Regression-Kriging}\label{regression-kriging}}
...
target variable and environmental covariates. The modelled relationship
between predictors and target are summarized in regression equation,
...
modelled using a regression model. Next, the prediction residuals are
...
major variables of interest. You can produce histograms for the
...
collinearity, or overlap, among independent variables can cause problems
in multiple linear regression models. Collinearity (also
multicollinearity) is a phenomenon in which two or more predictors in a
multiple regression model are highly correlated. Collinearity causes
...
regression models. The multicollinearity of number of variables can be
...
performed using any commercial or open source statitical tool (R, SPSS,
MINITAB\ldots{})
\end{quote}

The main steps for the multiple linear regression analysis are shown in
the Figure {[}fig:Workflow for Regression Kriging{]}.

\begin{figure}
\centering
\includegraphics{images/RKworkflow.png}
\caption{Workflow for Regression Kriging}
\end{figure}
...
information to a given soil depth (0-30 cm) in the section 2.4. Spline
function basically imports soil profile data (including instances where
layers are not contiguous), fits it to a mass-preserving spline and
outputs attribute means for a given depth. The output file should
contain profile id, upper (surface) and lower depth (30cm), estimated
value for the selected soil attribute (Value) and tmse (estimated mean
squared error of the spline). If you used the Spline Tool V2, the
coordinates were not kept in the output file. The coordinates should be
added back in the data table. You can use Profile IDs to add the X, Y
columns back. Once your point dataset is ready, copy this table into
...
\begin{verbatim}
## 'data.frame':    2893 obs. of  22 variables:
...
##  $ soilmap : Factor w/ 20 levels "1","2","3","4",..: 6 6 14 3 3..
\end{verbatim}
...
covariates required for the modelling. In this case, we will subset the
columns SOC, the covariates and the the spatial coordinates (X and Y).
...
Lets fit a new model using a random subset of the available data. We
...
the goof() function from the ``ithir'' package.
...
\includegraphics{SOCMapping_files/figure-latex/unnamed-chunk-35-1.pdf}
...
## soilmap 3.827741 4.358899        1.035954
\end{verbatim}
...
## F-statistic: 29.32 on 30 and 2862 DF,  p-value: < 2.2e-16
\end{verbatim}
...
## 217  -4.750215         2.1322e-06   6.1684e-03
\end{verbatim}
...
into table format. If multiple rasters are being used, it is necessary
...
\includegraphics{SOCMapping_files/figure-latex/unnamed-chunk-43-1.pdf}
...
\includegraphics{SOCMapping_files/figure-latex/unnamed-chunk-44-1.pdf}
...
\hypertarget{data-mining-random-forest}{%
\section{Data mining: Random Forest}\label{data-mining-random-forest}}
...
is used on digital soil mapping frameworks to generate spatial and
...
framework, the exploration of statistical relationships (lineal and
non-lineal) between soil observational data and soil environmental
...
digital soil mapping. Machine learning methods can be roughly divided in
four main groups: linear-based (e.g., multiple linear regression),
...
Random forest is a Desicion-tree-based machine learning method useful on
digital soil mapping for uncovering statistical relationship between a
dependent variable (e.g., soil property) and its predictors.
...
(Figure 6.14). The name random forest originates from the fact that the
original data is first randomly split into sub-samples, and many
decision trees (or forest) are used to model the sub-samples.

\begin{figure}
\centering
\includegraphics{images/randomForestconcept.png}
\caption{The concept of Random Forest and Decision Trees}
\end{figure}
...
because its prectically free of assumptions. Random Forest has shown
accuracy of spatial predictions because the random selection of subsets
and prediction factors reduce potential over-fitting and data noise
\citep{wiesmeier2011digital}. Over-fitting and data noise are important
uncertainty sources across high dimensionally spaces used to represent
the soil forming environment. Thus, the advent of open-source platforms
and freely downloadable ancillary data (e.g., worldgrids.org) to
represent the soil forming environment makes of random forest and other
such models increasingly appealing on digital soil mapping.

To predict continuos data (such as carbon density) Random forest
generates a averaged ensemble of regression trees based on bagging,
...
1996, 2001). Each tree contributes to weigth the statistical
...
which allows to assess the relatively importance of the available
...
prediction to new data is the weigthed average of all generated trees.
...
usually represented in a tabular form or a geo-spatial object (e.g.,
...
object oriented software that rely on specific-contributor libraries.
There are several libraries for the implementatio of the Random forest
...
study area and a statistical criteria (e.g., just to use the best
correlated predictors). Multivariate analysis (e.g., principal component
analysis) are a widely used approach to identify informative predictors.
Here we use this combination of prediction factors to be consistent with
other book chapters and because they were previously selected for this
exercise using expert knowledge about the spatial variability of soil
organic carbon. Now, we will build a working hypothesis from our
conceptual model, using all the continous prediction factors for
\emph{OCSKGMlog}:
...
for modeling, because not all categories in the map are represented by
available \emph{OCSKGM} estimations, therefore this map requires a
...
balanced. Another alternative to use a unbalanced categorical map is by
generating dummy variables, where each category in the map becomes an
indepentent binomial predictor variable (e.g., only 0 and 1 values) as
is explained in the following chapter. The risk of doing so rely in the
potential underestimation of the spatial variability of the target
variable under each category with a low density of available data.

\textbf{Step - Tuning parameters}
...
selected with the number of trees when this relationships stabilizes at
...
\NormalTok{rfmodel <-}\StringTok{ }\KeywordTok{train}\NormalTok{(fm, }\DataTypeTok{data=}\NormalTok{dat}\OperatorTok{@}\NormalTok{data, }\DataTypeTok{method =} \StringTok{"rf"}\NormalTok{, }\DataTypeTok{trControl =}\NormalTok{ ctrl, }
             ...
             \CommentTok{#This is a very useful function to compare and test different prediction algorithms}
             \CommentTok{#type names(getModelInfo()) to see all the possibilitites implemented on this function}
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\includegraphics{SOCMapping_files/figure-latex/unnamed-chunk-52-1.pdf}
\caption{\label{fig:unnamed-chunk-52}Model Decreasing Error and Node Purity}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{SOCMapping_files/figure-latex/unnamed-chunk-53-1.pdf}
\caption{\label{fig:unnamed-chunk-53}select ntree}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{SOCMapping_files/figure-latex/rf prediction-1.pdf}
\caption{(\#fig:rf prediction)SOC prediction using a randomForest model}
\end{figure}
...
uncertainty. The uncertainty can be roughly divided on four main
...
generation and prediction is made on each split, in a way that the
...
predictions allows to generate a map of model sensitivity to available
data.
...
\CommentTok{# distribution of OCSKGMlog, note that we are using the mtry parameter that was selected by }
\CommentTok{#the train funtion of the caret package, assuming that the 75% of data previously used well resembles the }
\CommentTok{#statistical distribution of the entire data population. }
\CommentTok{#Otherwise repeat the train function with all available data (using the object dat that instead of train) to select mtry.}
\CommentTok{#(mod <- train(fm, data=dat, method = "rf", trControl = ctrl, importance=TRUE))}
...
distribution calculated for each pixel, which represent the extreme
...
We have created two maps in the working directory, one represent the
...
\hypertarget{data-minning-support-vector-machines}{%
\section{Data minning: Support Vector
Machines}\label{data-minning-support-vector-machines}}

\emph{GF Olmedo \& M Guevara}

Support vector machines (svm) is a machine learning technique used for
classification and regresion analysis. According to \citet{scikit} the
...
dimensional spaces (were the dimesions are the covariates) with a
...
problems, like the modelling of SOC using enviromental covariates. When
svm is used to solve regression problem is called support vector
regression.

For doing support vector regression, svm applies a simple linear method
to the data but in a high-dimensional feature space non-linearly related
to the input space. Then, it creates a hyperplane through n-dimensional
spectral-space. And separates numerical data based on a kernel function
...
possible margin, being the support vectors the points which fall within.
Then, linear models are fitted to the support vectors
\citep{soil-2017-40}.
...
svm is a huge area of study, we can recommend the following books:
\citet{vapnik2013nature}, \citet{friedman2001elements} and
\citet{james2013introduction}.

\hypertarget{technical-steps---fitting-a-svm-model-to-predict-the-soc}{%
\subsection{Technical Steps - Fitting a SVM Model to Predict the
SOC}\label{technical-steps---fitting-a-svm-model-to-predict-the-soc}}
...
\NormalTok{selectedCovs <-}\StringTok{ }\KeywordTok{cor}\NormalTok{(}\DataTypeTok{x =} \KeywordTok{as.matrix}\NormalTok{(dat}\OperatorTok{@}\NormalTok{data[,}\DecValTok{5}\NormalTok{]),}
           \DataTypeTok{y =} \KeywordTok{as.matrix}\NormalTok{(dat}\OperatorTok{@}\NormalTok{data[,}\OperatorTok{-}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{6}\NormalTok{,}\DecValTok{12}\NormalTok{,}\DecValTok{20}\NormalTok{)]))}
...
\begin{verbatim}
## [1] "OCSKGM"  "B04CHE3" "TMDMOD3" "DEMENV5" "B07CHE3" "PRSCHE3"
\end{verbatim}
...
\begin{verbatim}
## [1] "B04CHE3" "TMDMOD3" "DEMENV5" "B07CHE3" "PRSCHE3"
\end{verbatim}
...
According to \citet{hsu2003practical}, svm requieres each variable to be
represented by a vector of real number. This means that factor
variables, like \texttt{covs\$LCEE10} and \texttt{covs\$soilmap}has to
be converted into numeric data. In statistics this kind of variables are
called boolean indicators or dummy variables. Dummy variables take a
value of 0 or 1 indicating the presence of ausence of an specific
...
the presence/ausence of every category. For converting our covariates to
...
We have to convert the collumns with categorical variables in the soil
...
## [41] "soilmap17" "soilmap18" "soilmap19" "soilmap20"
\end{verbatim}
...
In this example, we will show how to tune 2 paramenters using a grid
search for hyperparameter optimization using the function
\texttt{tune()}. The first parameter is epsilon wich is the
insensitive-loss function. The default value for epsilon is 0.1, and we
will try 11 different value from 0.05 to 0.12 in 0.1 increments. the
second parameter is the cost wich is the cost of constraints violation
-- it is the `C'-constant of the regularization term in the Lagrange
formulation. The default value for this parameter is 1, and we will try
values from 1 to 20 in 5 increments. The value of cost helps us to avoid
overfitting. this is a very heavy step in computational terms and can
take a lot of time, because we will have to try a lot of different
model.
...
\NormalTok{  tuneResult <-}\StringTok{ }\KeywordTok{tune}\NormalTok{(svm, OCSKGM }\OperatorTok{~}\NormalTok{.,  }\DataTypeTok{data =}\NormalTok{ dat}\OperatorTok{@}\NormalTok{data[,}\KeywordTok{c}\NormalTok{(}\StringTok{"OCSKGM"}\NormalTok{,}
                                                         \KeywordTok{names}\NormalTok{(COV))],}
                     \DataTypeTok{ranges =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{epsilon =} \KeywordTok{seq}\NormalTok{(}\FloatTok{0.08}\NormalTok{,}\FloatTok{0.16}\NormalTok{,}\FloatTok{0.02}\NormalTok{),}
                                   \DataTypeTok{cost =} \KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{5}\NormalTok{,}\DecValTok{7}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\includegraphics{SOCMapping_files/figure-latex/unnamed-chunk-72-1.pdf}
\caption{\label{fig:unnamed-chunk-72}Performance of the different svm models
...
The best model is chosen as the one with the lowest mean squared error
using cross validation. The parameters for the cross validation can be
defined in the \texttt{tune.control()} function. By default it uses
cross-validation using 10 partitions.
...
## 
## 
## Number of Support Vectors:  2021
\end{verbatim}
...
\begin{figure}
\centering
\includegraphics{SOCMapping_files/figure-latex/unnamed-chunk-74-1.pdf}
\caption{\label{fig:unnamed-chunk-74}SOC prediction using a support vector
machines model}
\end{figure}
...
\begin{verbatim}
##    B04CHE3   soilmap6   soilmap5    TMDMOD3    B07CHE3 
...
##  3.2918996  3.2883060  0.7698098  0.1110606
\end{verbatim}
...
contain errors. The magnitude of the errors determine the quality of the
map. If a map matches reality well (the error is small), the quality or
accuracy of the map is high. On the other hand, if a map does not match
reality well, map accuracy is low.
...
yields a complete probabilistic characterisation of the map uncertainty,
but such characterisation is only valid under the assumptions made;for
...
the mean squared error and root mean squared error, the model efficiency
...
Map quality measures are therefore, defined as population means. Because
we cannot afford to determine the prediction error at each location
(grid cell) of the mapping area to calculate the population means, we
have to take a sample of a limited number of locations in the mapping
area. This sample is then used to estimate the population means. It is
important to realize that we are uncertain about the population means,
because we estimate it from a sample. Ideally this uncertainty is
...
descriptions of the quality measures from which various parameters can
be reported, such as the mean, median or percentiles. Furthermore, they
...
squared prediction errors, but here the mean of the area is used as
...
of the data set as predictor for any location in the mapping area. An
\({AVE}\) value larger than zero (\({RSS}\) smaller than \({TSS}\))
means that the model predictions are an improvement over using the mean
as predictor (this is what we hope for). In case the \({AVE}\) is
...
locations in the mapping area in a two-way contingency table (Figure
8.1). The population error matrix is a square matrix of order \(U\),
with \(U\) being the number of soil classes observed and mapped. The
columns of the matrix correspond to observed soil classes and the rows
to predicted soil classes (the map units).\(N\) is the total number of
locations of the mapping area. Elements \(N_{ij}\) are the number of
locations mapped as class \(i\) with observed class \(j\). The row
margins \(N_{i+}\) are the locations mapped as class \(i\), and column
margins \(N_{+j}\) the locations for which the observed soil class is
\(j\). Note that the elements of the population error matrix can also be
interpreted as surface areas. In that case element \(N_{ij}\) is the
surface area mapped as class \(i\) with observed class \(j\).
...
and spatial bubble plots of validation errors. Figure 8.2 shows an
example of a scatterplot and bubble plot. Both plots can be easily made
with R (R Development Core Team, 2016). Use the function plot(x,y)to
generate a scatter plot. The 1:1 line (black line in Figure 8.2) can be
added to the plot with the command abline(0,1). The spatial bubble plot
can be generated with the bubble function of the sp package
\citep{pebesma2005classes}.

\begin{figure}
\centering
\includegraphics{images/Validation_Rwanda.png}
\caption{Scatterplot of predicted versus observed soil organic matter
content for Rwanda (left) and spatial bubble plot of cross-validation
error for soil organic matter (right) (Kempen et al., 2015). The black
line in the scatter plot represents the 1:1 line of prediction versus
observed, the blue line represents the regression between observed and
predicted values}
\end{figure}
...
quality measures introduced in previous section from a sample.

With additional probability sampling an independent dataset is collected
from the sampling population (all grid cells of a digital soil map) for
the purpose of validation. This dataset is used in addition to a dataset
that is used to calibrate a prediction model. Such dataset is often a
legacy dataset collected with a purposive sampling design.
...
\item
  ...
  measures can be obtained that allow for hypothesis testing, e.g.~for
...
In simple random sampling, no restrictions are imposed on random
...
accessed. Irregularity, clustering and open spaces characterise the
...
prediction error. A high \(R^2\)-value therefore, does not automatically
mean a high \(AVE\). For instance, in case of strongly biased
predictions the \(R^2\) can be high but the \(AVE\) will be low. The
blue line in Figure 8.2 is the regression line that one obtains when
regression the observed value on the predicted value. This line slightly
differs from the 1:1 line. In this example the \(R^2\) of the regression
is 0.42 while the \(AVE\) is 0.40. The uncertainty associated to the
estimated map quality measures is quantified with the sampling variance,
which for the \(ME\), \(MAE\) and \(MSE\) is estimated by:
...
validation data (Figure 8.3). \(n\) is the total number of validation
locations in the sample. Element \(n_{ij}\) of the matrix corresponds to
the number of validation locations that have been predicted as class
\(i\), \(i = 1, 2, \dots U\) and belong to class \(j\),
\(j = 1, 2, \dots U\) \citep{lark1995components}. The matrix summarizes
correct predictions and incorrect predictions within the validation
data.

\begin{figure}
\centering
\includegraphics{images/Validation_error_matrix.png}
\caption{Sample error matrix.}
\end{figure}
...
\(CI\). Figure 9.4 shows a hypothetical example of a sample error matrix
for soil class map. For this example, the overall purity is estimated
by:
...
\begin{figure}
\centering
\includegraphics{images/Validation_error_matrix2.png}
\caption{Sample error matrix for a hypothetical soil class map.}
\end{figure}

Table 9.1 gives the map unit purities and class representations for this
example. The map unit purity of the Gleysol is 0.581, meaning that at
\(58.1\%\) of the validation locations for which a Gleysol is predicted,
a Gleysol is observed. Assuming the validation data were collected by
simple random sampling, we could conclude that for \(58.1\%\) of the
area mapped as Gleysol we would find a Gleysol in the field. The class
representation of the Gleysol is 0.463, meaning that for \(46.3\%\) of
the validation locations classified as Gleysol, we map a Gleysol. The
majority of the Gleysol locations is thus mapped as a different soil
class. Again, assuming the validation data were collected by probability
sampling, we would estimate that \(22.5\%\) (\(\frac{54}{240}\)
\(\times\) \(100\%\)) of our mapping area is covered by Gleysols. We map
Gleysols for \(17.9\%\) of the area (\(\frac{43}{240}\) \(\times\)
\(100\%\)). It can happen that a soil class has a high map unit purity
and a low class representation. This means that if we map a Gleysol we
will likely find a Gleysol there, but that a large extent of the true
Gleysol area is not mapped as such.

\textbf{Table 9.1.} Map unit purity and class representation statistics
for the hypothetical example given in Figure 9.4.

\begin{longtable}[]{@{}lrr@{}}
\toprule
& map unit purity & class representation\tabularnewline
\midrule
\endhead
Anthrosol & 0.679 & 0.633\tabularnewline
Cambisol & 0.508 & 0.516\tabularnewline
Gleysol & 0.508 & 0.463\tabularnewline
Luvisol & 0.592 & 0.700\tabularnewline
Podzol & 0.576 & 0.594\tabularnewline
\bottomrule
\end{longtable}
...
In data-splitting the sample data set is split in two subsets. One
...
\hypertarget{technical-steps---validation}{%
\subsection{Technical steps -
Validation}\label{technical-steps---validation}}

\emph{GF Olmedo}

\hypertarget{prediction-error}{%
\subsubsection{Prediction error}\label{prediction-error}}

First, we will load the test dataset. This dataset was measured at the
same time as the modelling dataset. After data preparation in chapter
\ref@(preparation), we used the code in section \ref@(dataSplit) to
split the database: 75\% of the points were used in the models from
chapter \ref@(mappingMethods) and now we will use the remaing 25\% of
the points to test the results.
...
Now, we will load the results from chapter \ref@(mappingMethods),
extract the predicted value for every point and then, estimate the
prediction error.
...
\subsubsection{Data-Splitting}\label{dataSplit}}
...
\begin{figure}
\centering
\includegraphics{SOCMapping_files/figure-latex/unnamed-chunk-78-1.pdf}
\caption{\label{fig:unnamed-chunk-78}Statistical distribution of train and
test datasets}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#Plot the observed vs modeled including a regression line}
\CommentTok{#This model was generated with the 75% of all data}
\KeywordTok{plot}\NormalTok{(test}\OperatorTok{$}\NormalTok{OCSKGMlog, test}\OperatorTok{$}\NormalTok{pred, }\DataTypeTok{main=}\StringTok{'validation plot'}\NormalTok{ )}
\KeywordTok{abline}\NormalTok{(}\KeywordTok{lm}\NormalTok{(test}\OperatorTok{$}\NormalTok{pred}\OperatorTok{~}\NormalTok{test}\OperatorTok{$}\NormalTok{OCSKGMlog), }\DataTypeTok{col=}\StringTok{'black'}\NormalTok{, }\DataTypeTok{xlab=}\StringTok{'observed'}\NormalTok{, }
       \DataTypeTok{ylab=}\StringTok{'modeled'}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
because soil spatial variation is complex and cannot be modelled
...
This chapter describes how uncertainty may be characterised by
...
difficult to arrive at a spatially explicit characterisation of
...
analysed for various soil properties. Let us further assume that the
...
sufficient duplicates and randomise the order in which soil samples are
analysed in the laboratory. This allows to quantify the combined field
...
relationship between the soil variable and covariates and deteriorate
the quality of the final soil map.
...
modelling or independent validation is important.
...
traced provided that the uncertainty sources are characterised
...
as \(35\%\), while recognising that a field estimate is quite crude and
...
\(2.5\%\). In general, statistical modelling is easier if the variables
...
statistical modelling. For instance, when building a DSM model of OC, it
...
and possibly time. Thus, the characterisation of uncertainty about soil
...
locations, depths or times) \(Z\) is fully characterised by the vector
of means \(m\) and variance-covariance matrix \(C\). Figure 7.1 shows
three examples of 500 paired soil property values that were simulated
from different bivariate normal distributions. The left panel shows an
uncorrelated case with equal standard deviations for both properties.
The centre and right panels show a case where soil property 2 has a
greater standard deviation than soil property 1. The difference between
these two cases is that the centre panel has a zero correlation between
the two soil properties while it is positive in the right panel.

\begin{figure}
\centering
\includegraphics{images/pairedsoilpropierties.png}
\caption{Scatter plots of 500 paired soil property values drawn from a
...
characterised by probability distributions, let us consider what these
...
a similar way as displayed in Figure 7.1.
...
\ref{eq:DSM model} is spatially autocorrelated, as characterised by a
...
values and positions of the soil point data, and in the covariates.
These sources of uncertainty will also affect the outcome of DSM model
predictions.
...
weak covariates leads to poor predictions of the soil (e.g.~Samuel Rosa
et al.~2015).
...
potential users can easily access it, use it, and make correct
...
The authors have 15+ years of experience with producing, publishing and
...
Most of soil data is in fact spatial. \textbf{``Spatial''} implies that
spatial (and temporal) reference is attached to each measured /
estimated value, i.e.~it is location specific. Spatio-temporal
references typically includes for example:
...
are often complemented with soil sample data and serve both data formats
. A Soil Information System should preferably be a \textbf{Database}
(DB), so that users can access and query data using some standard DB
languages (for example SQL). Steps to export soil data into a DB format
...
putting a GPKG file on server with read and execute access allows users
to connect and fetch data.

Alternatively, it is also good idea to store point data in non-spatial
format such as simple tables. For example, in comma-separated file
format (.csv). A fast way to publish and share tabular data is to use
Google Fusion Tables™. Google Fusion Tables have an API that allows
accessing and using tabular data through various programming platforms.
The limitation of using Google Fusion tables is however, data size
(currently about 1GB per user) and similar data volume limits, so this
platform should be only used as intermediate solution for smaller data
sets.

\begin{figure}
\centering
\includegraphics{images/Data_sharing_eberg.png}
\caption{Displaying point data set eberg (used in the previous example)
...
Consider for example the \textbf{gdallocationinfo} function which allows
spatial queries following some indexing system such as row and column
number:
...
you could have a robust solution for sharing your data to thousands of
users.
...
adhering to the data sharing agreement and licence set by the data
producer.
...
for serving raster or vector data. It includes majority of the Open
...
Very important functionality of GeoServer are the OGC standard services
such as the \textbf{Web Coverage Service} (WCS) and the \textbf{Web
Feature Service} (WFS). WCS means that not only data views are available
to users, but WCS can also do data translation, aggregation or
resampling, overlay etc. Consider for example the SoilGrids WCS (which
can also be opened in QGIS or similar software supporting WCS). Users
can direct this WCS and request only a subset of data for some area,
aggregated to some preferred resolution / pixel size by using
gdal\_translate or similar. This means that, in few steps, users can
download a subset of data on GeoServer in a preferred format without a
need to download the whole data set.
...
definitively consider, as a way to boost usability of your data are:
...
projections can be managed using open source tools such as GIS
softwares, GDAL tools suite (\url{http://www.gdal.org/}) and various
packages of the R software (\url{https://www.r-project.org/}).
Projections can be defined according to different standards. A common
way is the EPSG database. EPSG Geodetic Parameter Dataset is a
collection of definitions of coordinate reference systems and coordinate
...
\item
  Do all the covariates have a resolution of 1 km, and did you use
  either nearest neighbour resampling for the categorical variables and
...
\item
  ...
  unrealistic pattern, based on expert judgement. If so, consider
...
\item
  ...
  consider modelling the 90\% confidence intervals of the predictions!
\item
...
using common GIS formats, and metadata should be compiled in an excel
...
\item
  ...
  30 arc-seconds resolution. An empty grid is provided by the GSP
...
\item
  ...
  Methods. The uncertainty associated to the estimated map quality
...
\chapter{Code examples}\label{code-examples}}

\emph{GF Olmedo}

\hypertarget{PreparationProfiles}{%
\section{Data Preparation for Soil Profiles}\label{PreparationProfiles}}
...
\hypertarget{PreparationAuger}{%
\section{Data Preparation for Top Soil or Auger
Samples}\label{PreparationAuger}}
...
\hypertarget{mixing-covariates-and-soil-points-data}{%
\section{Mixing Covariates and Soil Points
Data}\label{mixing-covariates-and-soil-points-data}}
...
\hypertarget{fitting-a-rk-model-to-predict-the-ocs}{%
\section{Fitting a RK model to predict the
OCS}\label{fitting-a-rk-model-to-predict-the-ocs}}
...
\hypertarget{fitting-a-random-forest-model-to-predict-the-soc}{%
\section{Fitting a random forest model to predict the
SOC}\label{fitting-a-random-forest-model-to-predict-the-soc}}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(reshape)}

\CommentTok{# Correlation analysis to select covariates}
\KeywordTok{names}\NormalTok{(dat)}
\NormalTok{COR <-}\StringTok{ }\KeywordTok{cor}\NormalTok{(}\KeywordTok{as.matrix}\NormalTok{(dat[,}\DecValTok{7}\NormalTok{]), }\KeywordTok{as.matrix}\NormalTok{(dat[,}\OperatorTok{-}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{8}\NormalTok{)]))}
\NormalTok{COR}
\NormalTok{x <-}\StringTok{ }\KeywordTok{subset}\NormalTok{(}\KeywordTok{melt}\NormalTok{(COR), value }\OperatorTok{!=}\StringTok{ }\DecValTok{1} \OperatorTok{|}\StringTok{ }\NormalTok{value }\OperatorTok{!=}\StringTok{ }\OtherTok{NA}\NormalTok{)}
\NormalTok{x <-}\StringTok{ }\NormalTok{x[}\KeywordTok{with}\NormalTok{(x, }\KeywordTok{order}\NormalTok{(}\OperatorTok{-}\KeywordTok{abs}\NormalTok{(x}\OperatorTok{$}\NormalTok{value))),]}
\NormalTok{x[}\DecValTok{1}\OperatorTok{:}\DecValTok{25}\NormalTok{,]}

\NormalTok{idx <-}\StringTok{ }\KeywordTok{as.character}\NormalTok{(x}\OperatorTok{$}\NormalTok{X2[}\DecValTok{1}\OperatorTok{:}\DecValTok{25}\NormalTok{])}

\NormalTok{dat2 <-}\StringTok{ }\NormalTok{dat[}\KeywordTok{c}\NormalTok{(}\StringTok{'OCSKGM'}\NormalTok{, idx)]}
\KeywordTok{names}\NormalTok{(dat2)}

\NormalTok{COVall <-}\StringTok{ }\NormalTok{COV}
\NormalTok{COV <-}\StringTok{ }\NormalTok{COV[[idx]]}

\KeywordTok{plot}\NormalTok{(COV)}


\KeywordTok{library}\NormalTok{(randomForest)}

\CommentTok{# Try different values of mtry and select the model with the optimal }
\CommentTok{# value}
\NormalTok{model <-}\StringTok{ }\KeywordTok{tuneRF}\NormalTok{(dat[,}\KeywordTok{c}\NormalTok{(}\KeywordTok{names}\NormalTok{(COV))], dat}\OperatorTok{$}\NormalTok{OCSKGM, }\DataTypeTok{stepFactor=}\FloatTok{1.5}\NormalTok{, }
                \DataTypeTok{doBest =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{improve =} \FloatTok{0.5}\NormalTok{)}
                       
\CommentTok{# Use the model to predict the SOC in the covariates space}
\KeywordTok{beginCluster}\NormalTok{()}
\NormalTok{start <-}\StringTok{ }\KeywordTok{Sys.time}\NormalTok{()}
\NormalTok{pred <-}\StringTok{ }\KeywordTok{clusterR}\NormalTok{(COV, predict, }\DataTypeTok{args=}\KeywordTok{list}\NormalTok{(model))}
\KeywordTok{print}\NormalTok{(}\KeywordTok{Sys.time}\NormalTok{() }\OperatorTok{-}\StringTok{ }\NormalTok{start)}
\KeywordTok{endCluster}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\hypertarget{fitting-a-svm-model-to-predict-the-soc}{%
\section{Fitting a svm model to predict the
SOC}\label{fitting-a-svm-model-to-predict-the-soc}}
...
\CommentTok{#  Test different values of epsilon and cost}
\NormalTok{tuneResult <-}\StringTok{ }\KeywordTok{tune}\NormalTok{(svm, OCSKGM }\OperatorTok{~}\NormalTok{.,  }\DataTypeTok{data =}\NormalTok{ dat[,}\KeywordTok{c}\NormalTok{(}\StringTok{"OCSKGM"}\NormalTok{, }
                                                  \KeywordTok{names}\NormalTok{(COV))],}
                   \DataTypeTok{ranges =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{epsilon =} \KeywordTok{seq}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{,}\FloatTok{0.1}\NormalTok{), }
                                 \DataTypeTok{cost =} \KeywordTok{c}\NormalTok{(.}\DecValTok{5}\NormalTok{,}\DecValTok{1}\NormalTok{,}\FloatTok{1.5}\NormalTok{,}\DecValTok{2}\NormalTok{,}\DecValTok{5}\NormalTok{,}\DecValTok{10}\NormalTok{))}
\NormalTok{)}
...
\KeywordTok{beginCluster}\NormalTok{()}
\NormalTok{start <-}\StringTok{ }\KeywordTok{Sys.time}\NormalTok{()}
\NormalTok{pred <-}\StringTok{ }\KeywordTok{clusterR}\NormalTok{(COV, predict, }\DataTypeTok{args=}\KeywordTok{list}\NormalTok{(tunedModel))}
\KeywordTok{print}\NormalTok{(}\KeywordTok{Sys.time}\NormalTok{() }\OperatorTok{-}\StringTok{ }\NormalTok{start)}
\KeywordTok{endCluster}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
