...
3. Start ‘QGIS desktop with GRASS’. Fig. \@ref(fig:qgis) shows the start screen of QGIS desktop. In the upper left panel, there is the browser panel, which lists the geodata used for this example. In the bottom left, the layer information is given for the layers displayed on the right.

```{r qgis, fig.cap="QGIS Desktop with the browser panel on the upper left, the layer information on the bottom left and the display of your layers on the right" , out.width='80%', echo=FALSE, fig.align='center'}
...
In **Step 2** you will make a SOC map, based on the FYROM Soil Map and the SOC values at the sampled points, following 3 steps: 1) Extract the soil map information for the point data, 2)  obtain the mean and standard deviation of the SOC stocks per soil class, based on the point data and 3) assign these values to the corresponding soil map units. The steps are detailed below:
...
```{r, fig.cap="Example field calculator" , out.width='80%', echo=FALSE, fig.align='center'}
...
**Step 4 - Joining landscape units and soil mapping units to support class- and geo-gatching (optional)**
