...
```{r qgis, fig.cap="QGIS Desktop with the browser panel on the upper left, the layer information on the bottom left and the display of your layers on the right"}
...
```{r layerprop, fig.cap="Changing layer properties for the FYROM Soil Map"}
...
```{r, fig.cap="Project properties and projection settings"}
...
```{r, fig.cap="Join attributes by location"}
...
```{r, fig.cap="Example field calculator"}
...
```{r, fig.cap="Calculate group statistics"}
...
```{r, fig.cap="Change the legend style to display the SOC values"}
...
```{r, fig.cap="Example of the Map composer"}
...
```{r, fig.cap="Change the projection of a raster file"}
