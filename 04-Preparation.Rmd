...
```{r}
dat_horiz <- read.csv(file = "data/horizons.csv")
...
summary(dat_horiz$CRF)

# Convert NA's to 0
dat_horiz$CRF[is.na(dat_horiz$CRF)] <- 0
...
$BD = 1,62-0,06 * OM$ Saini (1996)

$BD = 1/(0,6268 + 0,0361 * OM)$ Drew (1973)

$BD = 1.482 - 0,6786 * (log OM)$ Jeffrey (1979)
...
summary(dat_horiz$BLD)

# See the summary of values produced using the pedo-transfer function with 
# one of the proposed methods.
summary(estimateBD(dat_horiz$SOC[is.na(dat_horiz$BLD)], method="Honeyset_Ratkowsky_1989"))

# Fill NA's using the pedotransfer function:
dat_horiz$BLD[is.na(dat_horiz$BLD)] <- estimateBD(dat_horiz$SOC[is.na(dat_horiz$BLD)], 
                                      method="Grigal_1989")

# explore the results
boxplot(dat_horiz$BLD)
```
...
depths(dat_horiz) <- ProfID ~ top + bottom
site(dat_horiz) <- dat_sites
```
