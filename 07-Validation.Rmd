...
First, we will load the test dataset. This dataset was measured at the same time as the modelling dataset. After data preparation in chapter \ref@(preparation), we used the code in section \ref@(dataSplit) to split the database: 75\% of the points were used in the models from chapter \ref@(mappingMethods) and now we will use the remaing 25\% of the points to test the results.
...
```{r summary of prediction error for 3 different methods, echo=FALSE}
...
knitr::kable(prederrors, caption = "Summary of prediction errors for 3 different methods", digits = 2, align = "lll")
```
...
```{r summary of map qualitity measures for 3 different methods, echo=FALSE}
...
knitr::kable(validation, caption = "Summary of prediction errors for 3 different methods", digits = 3, align = "lll")
```
...
```{r, eval=TRUE, fig.cap='Statistical distribution of train and test datasets'}
library(caret)
