...
```{r, fig.cap='Comparision of DSM model correlations (RK, RF, SVM) und statical distributions'}
library(psych)
...
Here we found that the higher Pearson correlation coefficient ($r$ value) between predicted values was between RK and SVM (0.86). We also found that the statistical distribution of predicted values is quite similar between the three methods and that the lowest correlation of predictions was found between the two machine learning approaches with $r$ 0.79 for RF and SVM.
...
To compare the performance of the three models, we will compare the observed values used and the predicted values for the the validation points. We have to load the validation dataset and the prediction result of the three models. The table containing these values was prepared in Section \@ref(TS:validation). 
...
**Step 2 - Calucalte statistical evaluation metrics**
