...
model <- quantregForest(y=dat@data$OCSKGMlog, x=dat@data[,8:20], ntree=500, 
                   keep.inbag=TRUE, mtry = as.numeric(rfmodel$bestTune))                        
```
...
writeRaster(exp(mean), file='results/MKD_OCSKGM_quantrf.tif', 
            overwrite=TRUE)
writeRaster(Total_unc_Percent, file='results/MKD_OCSKGM_rf_uncert.tif',
            overwrite=TRUE)
```
