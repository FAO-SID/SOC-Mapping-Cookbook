...
* OneGeology: The world geological maps are now being integrated via the OneGeology project which aims at producing a consistent Geological map of the world in approximate scale 1:1M [@jackson2007onegeology] http://www.onegeology.org/.
...
* @hartmann2012new have assembled a global, purely lithological database called GLiM (Global Lithological Map). GLiM consists of over 1.25 million digital polygons that are  classified in three levels (a total of 42 rock-type classes). https://www.geo.uni-hamburg.de/en/geologie/forschung/geochemie/glim.html).
...
```{r}
# load the lcimae covariates from the raster tif files
...
Combine the covariates provided with the raster version of the soil map prepared on subsection [Technical Steps - Rasterizing a vector layer in R].

```{r, fig.cap="Macedonia soil covariates"}
