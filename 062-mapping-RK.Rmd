...
* **Dependent variable $Y$**: \index{dependent variable} What we are trying to predict (e.g. SOC content).
* **Independent variables $X$'s (Predictors)**: Variables that we believe influence or explain the dependent variable (Covariates: environmental covariates, DEM derived covariates, soil maps, land cover maps, climate maps). The data sources for the environmental predictors are provided in Chapter \@ref(covariates).
...
We previously prepared the point dataset in \@ref(preperation) chapter. We will be using this data file (.csv) in this section.
...
Let's fit a new model using a random subset of the available data. We will sample 70% of the SOC data for the model calibration data set.
...
```{r predRK, fig.cap = "SOC prediction map for FYROM using a regression kriging model"}
plot(RKprediction)
```
