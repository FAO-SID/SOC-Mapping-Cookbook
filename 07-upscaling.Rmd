...
* [Preparation of spatial covariates](Environmental predictors)
  ...
  + Relief [DEM-derived covariates](e.g. DEM, Slope, TWI)
  
  + Organism map [Land cover/Land use](e.g. land use, NDVI, land cover)
  
  + Climate Data [Climate](e.g. mean precipitation, mean temperature)
...
```{r}
## Example of removing covariate4 from the stepwise model:
# modelo.MLR.step <- update(modelo.MLR.step, . ~ . - covariate4)
# use that code to remove the covariates with sqrt(vif) > 2
# then, you can test the vif again:
# sqrt(vif(modelo.MLR.step)) 
```
...
dat_sp <- spTransform(dat_sp, CRS("+init=epsg:3405"))

# project covariates to VN-2000 UTM 48N
COV <- projectRaster(COV, crs = CRS("+init=epsg:3405"), method='ngb')
```
...
writeRaster(RKprediction, filename = "URU_OCS_RK.tif")
writeRaster(RKpredsd, filename = "URU_OCS_RKpredsd.tif")
```
